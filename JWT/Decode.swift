...
func load(jwt:String) -> LoadResult {
  ...
  let header = NSJSONSerialization.JSONObjectWithData(headerData!, options: NSJSONReadingOptions(rawValue: 0)) as? Payload
  if header == nil {
  ...
  let payload = NSJSONSerialization.JSONObjectWithData(payloadData!, options: NSJSONReadingOptions(rawValue: 0)) as? Payload
  if payload == nil {
